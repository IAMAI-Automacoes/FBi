...
export default function Reports() {
  ...
  // O visual novo (batendo com a referência) é o que renderiza; o antigo
  // fica pronto ali embaixo, só desligado — nenhum dos dois foi apagado, e
  // como os dois recebem as MESMAS props (calculadas uma vez aqui), não tem
  // como duplicar estado nem um pisar no outro.
  const LAYOUT_NOVO = true
  return LAYOUT_NOVO ? <LayoutNovo {...layoutProps} /> : <LayoutAntigo {...layoutProps} />
}
...
/** Visual antigo — desativado (ver `LAYOUT_NOVO` acima), mantido intocado
 *  pra não perder nada: inclui o card de leitura por IA e os recortes em
 *  texto corrido que não estão na referência nova. */
function LayoutAntigo({
  period, setPeriod, semDados, kpis, stats, tendencia,
  analise, resumoExecutivo, analisando, handleAnalisar,
  gerandoPdf, gerandoCsv, handleExportCSV, handleExportPdf, diasComDados,
}: LayoutProps) {
  return (
...
// ── Visual novo (bate com a imagem de referência) ─────────────────────────

/** Ícone por categoria — categoria não mapeada cai no genérico (reticências). */
const ICONE_CATEGORIA: Record<string, any> = {
  'Tempo de espera': Clock,
...
function iconeCategoria(nome: string) {
  return ICONE_CATEGORIA[nome] ?? MoreHorizontal
}
          ...
          <CardNovo>
            ...
            <div className="mt-4 flex flex-col gap-6 sm:flex-row">
              <div className="shrink-0 sm:w-40">
                <p className="text-3xl font-bold text-gray-900">{kpis.sentiment}%</p>
                <p className="mt-0.5 text-sm text-gray-500">Satisfação atual</p>
                <TrendIndicator
                  trend={kpis.sentimentTrend} hasPrevData={kpis.hasPrevData} prevConfiavel={kpis.prevConfiavel}
                  prevTotal={kpis.prevTotal} suffix="vs. período anterior" isPontosCsat className="mt-2"
                />
              </div>
