 ...
 *   2. Mesmo `tema_id` de um insight ativo -> liga direto, zero IA — mas SÓ se
 *      não houver ação aberta na categoria. Havendo, a IA decide (ação tem
 *      preferência) e o insight do tema fica de reserva.
 ...
 *
 * ## Ação: só o vínculo. Insight: vínculo e, se precisar, texto novo
 *
 * A ordem é a regra do dono: primeiro ação, depois insight, e só sem nenhum
 * dos dois o feedback fica livre.
 *
 * Na AÇÃO o feedback é apenas ligado. O texto dela é o plano que a equipe já
 * está executando, e reescrevê-lo a cada relato novo mudaria o combinado no
 * meio do caminho. (Até aqui uma segunda IA reescrevia título e plano da ação;
 * saiu por decisão do dono.)
 *
 * No INSIGHT o vínculo paga uma segunda chamada de IA, que relê TODOS os
 * feedbacks já ligados e ajusta título, descrição e prioridade para englobar o
 * conjunto — um insight que diz "3 clientes reclamaram do tempero" com um
 * quarto relato de alguém que passou mal por baixo mentiria sobre a urgência.
 * Categoria e assunto_chave nunca são tocados: são a identidade do agrupamento.
 * É falha-segura: se a chamada quebrar, o vínculo fica e o texto também.
 *
 * ## Triagem
 *
 * O feedback só passa a contar como livre (ou ligado) para o resto do sistema
 * quando esta função termina e grava `triado_em`. Antes disso a view
 * `feedbacks_livres` e a geração de insights não o enxergam — é o que impede a
 * rodada da hora cheia de usar um feedback que ainda ia ser ligado a uma ação.
 */
...
/**
 * Encerra a triagem: a partir daqui o feedback conta como livre ou ligado para
 * o resto do sistema (`triado_em`, migration 20260913010000). Vem SEMPRE antes
 * de `talvezGerarInsights`, senão a contagem que decide a rodada não o enxerga.
 */
async function concluirTriagem(db: Db, feedbackId: number | string) {
  const { error } = await db
    .from('feedbacks_restaurante')
    .update({ triado_em: new Date().toISOString() })
    .eq('id', feedbackId)
    .is('triado_em', null)
  if (error) console.error('Falha ao concluir triagem:', error)
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (fb.tema_id) {
      ...
      // 1b. Não há ação do mesmo tema. Há insight ativo do mesmo tema?
      // deno-lint-ignore no-explicit-any
    ...
    //
    // A regra do dono é AÇÃO antes de INSIGHT. Antes, bastava o tema bater com um
    // insight da tela para o feedback ir para ele — sem nem olhar as ações
    // abertas da categoria, uma das quais podia resolver exatamente aquilo.
    if (vivoDoTema && acoesCat.length === 0) {
      const abs = await ligarAoInsight(db, fb, vivoDoTema.id, texto)
      await concluirTriagem(db, fb.id)
      return json({ status: 'ligado', destino: 'insight', id: vivoDoTema.id, via: 'tema', absorcao: abs, alerta })
    }
    // Havendo ações, a IA decide. O insight do tema entra na lista dela mesmo
    // que a categoria dele tenha saído diferente, e fica como reserva.
    if (vivoDoTema) {
      const idDoTema = vivoDoTema.id
      // deno-lint-ignore no-explicit-any
      if (!insightsCat.some((i: any) => i.id === idDoTema)) {
        // deno-lint-ignore no-explicit-any
        const naLista = (insights ?? []).find((i: any) => i.id === idDoTema)
        if (naLista) insightsCat.unshift(naLista)
      }
    }

    // Quando nenhuma ação é escolhida: o insight do tema, se houver; senão, livre.
    const reservaOuLivre = async (motivo: string) => {
      if (vivoDoTema) {
        const abs = await ligarAoInsight(db, fb, vivoDoTema.id, texto)
        await concluirTriagem(db, fb.id)
        return json({ status: 'ligado', destino: 'insight', id: vivoDoTema.id, via: 'tema-reserva', motivo, absorcao: abs, alerta })
      }
      await concluirTriagem(db, fb.id)
      await talvezGerarInsights(db, fb.restaurante_id)
      return json({ status: 'livre', motivo, alerta })
    }

    // ---- 3. Sem candidato nenhum: fica livre, sem gastar IA ----
    ...
    // ---- 4. Com candidatos: uma chamada de IA (ação tem preferência) ----
    const prompts = await carregarPrompts(db)
    const params = await paramsDoAgente(db, AGENTE, { max_tokens: 400 })
    if (!params) return await reservaOuLivre('agente desativado')
    ...
    try {
      ...
      if (err instanceof ErroCota) return await reservaOuLivre('sem credito')
      console.error('Falha ao decidir vinculo:', err)
      return await reservaOuLivre('erro na IA')
    }
    ...
    return await reservaOuLivre(decisao.motivo ?? 'nenhum candidato serve')
  } catch (err) {
    // deno-lint-ignore no-explicit-any
    const e = err as any
...
// deno-lint-ignore no-explicit-any
async function ligarAAcao(db: Db, fb: any, acaoId: number, _texto: string) {
  ...
  // Só o vínculo: o plano da ação é o que a equipe está executando.
  return { absorvido: false as const, motivo: 'ação só recebe o vínculo' }
}
